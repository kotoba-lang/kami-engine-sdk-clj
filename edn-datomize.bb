...
;; kami-engine-sdk-clj 用に com-junkawasaki/root superproject の
;; manifest/edn-datomize.bb (Phase 1/2 "EDN datomize" ワークストリーム) から
;; gftdcojp/net-kotobase の Phase 2 pilot 版経由で移植。schema-path をこの repo
;; のルート schema.edn に向けている以外はロジック同一。
;;
...
;;
;; マップ1個のファイルは [{...:db/id -1}] に包む。wrap-map はファイル種別ごとの
;; 名前空間を全キーに強制するが、wrap-generic は既に名前空間付きのキーはそのまま
;; 保持し、裸のキーにだけ default-ns を付与する（このリポジトリの
;; resources/kami/render/authority.edn のように idiomatic に :authority/*・
;; :render/* 等が付いている場合はこちらを使う）。値が Datomic の scalar
;; valueType（string/long/double/boolean/keyword、またはそれらの集合）に収まら
;; ないもの（入れ子 map、map を含む vector 等）は pr-str した文字列として保持
;; する（valueType=string の "blob" 属性にする — トップレベルの entity+attribute
...
;;   bb edn-datomize.bb wrap-generic <path> <default-ns>  — 名前空間付きキー混在のファイルを変換
;;   bb edn-datomize.bb adr-dir  <dir>                     — ADR frontmatter/body を変換（このリポジトリでは未使用）
;;   bb edn-datomize.bb adr-file <path>                    — ADR 1ファイルを変換（このリポジトリでは未使用）
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file"     (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                       ...
                       (merge-schema! (:attrs @report))
                       (println @report))
      (do (println "usage: bb edn-datomize.bb [wrap-map <path> <ns> | wrap-generic <path> <default-ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
